...
phases = [r'$\delta$-Phase', r'$\gamma$-Phase', r'$\beta$-Phase', r'$\alpha$-Phase']

energy_values = [delta_phase, ortho_phase, beta_phase, cubic_phase]
...
fig, (ax1, ax2) = pl.subplots(2, 1, figsize=(6, 12))


# Define font size for all labels and titles
font_size = 16

# Plot 1: Absolute energy values
ax1.bar(phases, energy_values, color=['skyblue', 'salmon', 'lightgreen', 'orange'])
ax1.set_xlabel('Phase', fontsize=font_size)
ax1.set_ylabel('Energy (KJ/mol)', fontsize=font_size)
ax1.set_title('Absolute Energy Values for Different Phases', fontsize=font_size + 2)
ax1.tick_params(axis='x', rotation=45, labelsize=font_size - 2)
ax1.tick_params(axis='y', labelsize=font_size - 2)
ax1.grid(axis='y', linestyle='--', alpha=0.7)

# Plot 2: Relative energy values to delta phase
ax2.bar(phases, relative_energy_values, color=['skyblue', 'salmon', 'lightgreen', 'orange'])
ax2.set_xlabel('CsPbI$_3$ polymorphs', fontsize=font_size)
ax2.set_ylabel('Relative Energy (KJ/mol)', fontsize=font_size)
ax2.set_title('Relative Energy Values with respect to Delta Phase', fontsize=font_size + 2)
